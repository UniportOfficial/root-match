...
## ⚠️ 핸드오프 이후 Session Update — Scenario B 완료 (2026-06-03 후속)

이 핸드오프 문서가 commit `84ffa8a`로 committed 된 후, **동일 세션 내에서 Scenario B가 실행되어 다음 항목들이 추가됨**. 본문 Quick Reference / 알려진 이슈 / 핵심 파일 인벤토리는 commit `84ffa8a` 시점 기준이므로, 다음 세션 시작 전 **이 섹션과 plan v0.7을 우선 확인**할 것.

### 현재 git 상태 (Scenario B addendum commit 이후)

```
브랜치: dev-monorepo (local-only commits, divergence count varies — verify via `git rev-list --count HEAD ^aa99d30` after the original Pre-Flight commit boundary)
HEAD: this Scenario B 클로저 addendum commit (commit subject:
       "docs(handoffs): scenario B closure addendum (Q3-Q6 RESOLVED, Q10 BLOCKER, §A delegation prompts ready)")
       — self-referential pointer (avoids stale SHA after amend); 검증은 `git log -1 --format='%H %s'`로
parent: 84ffa8a docs(handoffs): phase 1.W2 pre-flight complete, neon gate pending
직전 HEADs: 84ffa8a → aa99d30 (pre-flight) → a2e1e64 (housekeeping) → 51102f3 (mvp roadmap)
push 상태: 모두 로컬, 미푸시 (사용자 정책)
```

### Scenario B 산출물 (모두 `.sisyphus/plans/phase-1-w2.md` v0.7 안에 있음 — gitignored)

- **Plan v0.6 → v0.7** (1165 lines → 1500+ lines, +400 이상). 4 librarian + 2 explore 백그라운드 에이전트 결과 fold-in. 정확한 line count는 `wc -l .sisyphus/plans/phase-1-w2.md`로 확인.
- **§11 Q3-Q6 ✅ RESOLVED** (Better Auth 1.6.13 source citations + production OSS evidence: Rybbit, Midday, ClassroomIO, Docmost, Daytona, Postiz, AFFiNE, scanopy, mosh-helpdesk, ztnet, nextcrm-app):
  - Q3 Neon CI 전략 → **하이브리드**: ephemeral Neon branch (W2-7 e2e gate via `neondatabase/create-branch-action@v6`) + Docker `pgvector/pgvector:pg16` services block (fast unit/integration)
  - Q4 Throttler storage → **in-memory for MVP single-instance** + explicit migration trigger to `@nest-lab/throttler-storage-redis@1.2.0` before horizontal scale
  - Q5 Better Auth `cookies.secure` → **`BETTER_AUTH_URL.startsWith('https://')` 기반** (Better Auth 1.6.13 documented hierarchy match) + production `crossSubDomainCookies` with placeholder `rootmatching.com` domain
  - Q6 `additionalFields` enum 동기화 → **Prisma enum as single SoT** + exported readonly tuple + boot-time `assertSameSet()` drift guard; `role.input: false` (server-managed) + `accountType.input: true` + `validator.input: z.enum(ACCOUNT_TYPE_VALUES)` (Better Auth converts array type to `z.any()`)
- **§11.1 Detailed Resolutions** 신규 섹션 — 4 subsections + combined `auth.config.ts` template (paste-ready, with PrismaService dual-pool architectural honest note).
- **🔴 Q10 NEW BLOCKER**: `.github/workflows/ci.yml:42` `node-version: 20` + `package.json:8` `engines.node ">=20.10.0"` — pnpm 11.3.0 requires Node ≥ 22.13 (`ERR_UNKNOWN_BUILTIN_MODULE: node:sqlite`). **다음 push 전 두 줄 YAML/JSON 수정 필요** (W2-7 step 0 또는 사전 housekeeping commit).
- **Q11 NEW** (deferred): Prisma `package.json#prisma` deprecation 경고 — Prisma 7 업그레이드 시 `prisma.config.ts`로 마이그레이션.
- **Appendix §A 신규**: §A.1 W2-1 + §A.2 W2-2 ready-to-paste delegation prompts. 사용자가 Neon §6 셋업 완료 시 §A.1을 그대로 `task(category="deep", load_skills=["git-master"], ...)` 호출에 붙여넣으면 됨.

### 다음 세션 진입 방법 업데이트

1. 본 핸드오프 §0 + §1 환경 확인 그대로 (Node 22 + Pre-Flight 7 gates 재검증)
2. **추가 사전 작업 (Q10 BLOCKER 처리)**: 다음 push 전 `chore(ci): bump CI + repo engines to node 22` commit으로 `ci.yml:42` `node-version: 20` → `22` + `package.json:8` `>=20.10.0` → `>=22.13.0` 두 줄 수정. 또는 W2-7 step 0에서 처리 (현재 §7.7.3 step 0에 fold-in 됨).
3. 사용자 Neon §6 셋업 (본 핸드오프 §2 그대로)
4. **Wave 1 진입**: `.sisyphus/plans/phase-1-w2.md` §A.1 W2-1 prompt 그대로 `task()` 호출 (12 sub-steps per §7.1.3 in atomic order, 5 high-level deliverables in §A.1 [EXPECTED OUTCOME])

### Oracle 검증 라운드 (audit 완전성)

Plan v0.7 작성 + Scenario B 클로저 과정에서 Oracle iterative verification 진행 (여러 라운드, VERIFIED 도달 시까지 반복). 주요 fix 카테고리:

- 초기 라운드: stale §7.2 cookie template (NODE_ENV → BETTER_AUTH_URL.startsWith('https://')), signup curl `role:'client'` → `accountType:'client'` + `SameSite=Strict` → `Lax`, §7.7 Q3 hybrid fold-in, accountType.input policy split (false → true + validator), Q10 line-ref + engines mismatch broader scope
- 중간 라운드: assertSameSet export, prismaClient placeholder → `prisma.client.ts` singleton import, §7.2.3.1 cross-ref 정규화 to §7.2.3 sub-step 1, Q10 BEFORE-NEXT-PUSH vs W2-7-step-0 reconciliation, §A intro Q10/Q11 status wording, plan agent retroactive `APPROVE_WITH_NOTE` 기록
- 후기 라운드: §A.2 numbering bug (1,2,2,3-12 → 1-13), prerequisite ↔ Note 2 dual-pool consistency, §A.1 model names (MatchingRequest/Recommendation → QuoteRequest/MatchRecommendation/FactoryEmbedding + 4 enums), §7.5.3 `role: client` → `role: admin` + `accountType: client`, 본 handoff addendum 자체

Plan `.sisyphus/plans/phase-1-w2.md` §16 v0.7 row의 (e) + (h) 서브-단락은 Oracle Round 1 + Round 2의 9 fix씩 itemized 기록을 가지고 있음. Round 3 이후는 본 핸드오프 addendum의 위 categorical 요약 + plan §16 (f) plan-agent retroactive validation + (g) 3 retroactive missing-phase notes로 정리되며 round-by-round itemization은 없음 (stable-reference language 채택 후 round count 자체가 staleness 위험으로 분류됨). 최종 VERIFIED 시점은 `git log -1 --format='%H'` 기준 현재 HEAD.

### Pre-Flight 7-gate re-verification 결과 (Scenario B item 2 = 0-B 증거)

Scenario B 실행 중 + 종료 직전 Pre-Flight gates를 Node 22.22.3 + pnpm 11.3.0 환경에서 직접 재실행한 결과 (HEAD: `d4211ce` 기준 — 또는 이후 amend 시점 HEAD; `git log -1`로 확인):

```text
node --version       → v22.22.3
pnpm --version       → 11.3.0
pnpm -r typecheck    → 3 workspaces (packages/shared, apps/api, apps/web) all Done
pnpm lint            → 0 errors, 3 pre-existing <img> warnings (Phase 0 잔재, 무해)
pnpm format:check    → All matched files use Prettier code style
pnpm --filter @rootmatching/api build       → nest build success
pnpm --filter @rootmatching/api exec prisma generate → success (Q11 prisma.config.ts 향후 마이그레이션 권고만 출력)
pnpm --filter @rootmatching/api test        → Test Suites: 3 passed, 3 total / Tests: 25 passed, 25 total
pnpm --filter @rootmatching/api test:e2e    → Test Suites: 1 passed, 1 total / Tests: 2 passed, 2 total
```

**7-gate verdict**: 모두 green. Scenario B 작업이 코드 변경을 발생시키지 않았음을 증명하며 (item 4 "no code changes outside .sisyphus/plans/" constraint 충족), 다음 세션이 W2-1 진입 전 동일 명령 재실행 시에도 동일 green 출력 기대.

### Documented 잔여 gaps (의도적으로 doc-only scope 안에서 미해결)

§16 v0.7 (g)에 명시 기록:

1. Pre-edit TDD acceptance-criteria list 미작성 (round 1의 9 drift fixes 예방 가능했을 수단)
2. Momus v0.7-delta re-review pending (plan footer는 여전히 Momus v0.3 verdict만 인용)
3. Post-§A drift check (§A.1/§A.2 EXPECTED OUTCOME vs §7.1/§7.2 sub-step counts 1:1 매핑 grep) 미실행 — 단, R8에서 §A.1의 5-vs-12 매핑 차이는 clarifying note로 명시

W2-1 fires 전에 Momus v0.7-delta re-review 1회 권장.
...
활성 브랜치: dev-monorepo (local-only; divergence varies — Quick Reference 기준은 핸드오프 commit 시점; 최신 상태는 상단 Session Update + `git log -5` 확인)
...
ULTRAWORK 모드로 진행하며 Oracle 반복 라운드 + Momus 2 라운드 검증을 거쳐 모든 단계가 ACCEPT/VERIFIED로 closed (v0.6 작성 당시 Oracle 8회 — 정확한 round 카운트는 본 핸드오프 commit 시점 기준이며, 이후 Scenario B 작업의 추가 round는 상단 Session Update의 "Oracle 검증 라운드" 섹션 + plan §16 v0.7 row 참조).
...
5. **Oracle 반복 라운드 (v0.6 작성 시점 카운트)**: 반복 NOT VERIFIED → VERIFIED 도달 (sessions `ses_176b95717ffe5br4AZ6M7PGKUI`, `ses_176ae909affe7y5V0MrmBSRljL`, `ses_176a89096ffeYpYTk7baOtLFrh`, `ses_176a353dbffeiyZ8TjtA5zepCS`). Scenario B 추가 라운드는 상단 Session Update의 "Oracle 검증 라운드" 섹션 + plan §16 v0.7 row 참조.
...
# 현재 상태 확인
git status                              # → working tree clean (divergence count varies — verify with git log -5)
git log --oneline -5
...
### 🟡 결정 필요 (Phase 1.W2 진행 중) — ⚠️ **이 표는 핸드오프 commit 시점 기준 (84ffa8a). Q3-Q6은 이후 Scenario B 작업에서 모두 ✅ RESOLVED — 상단 Session Update §11 RESOLVED 항목 + plan v0.7 §11 + §11.1 참조. 본 표는 historical만 — superseded by Session Update.**

| Q   | 항목                                                                | deadline         | 현재 상태 (Scenario B 이후)                                 |
| --- | ------------------------------------------------------------------- | ---------------- | ----------------------------------------------------------- |
| Q3  | Neon CI branching 전략 (persistent vs ephemeral vs docker postgres) | W2-7 전          | ✅ RESOLVED — 하이브리드 (ephemeral Neon + Docker pgvector) |
| Q4  | Throttler storage backend (in-memory vs Redis)                      | W2-6 sub-track a | ✅ RESOLVED — in-memory MVP + Redis at horizontal scale     |
| Q5  | Better Auth `cookies.secure` (env-driven)                           | W2-2 sub-step 1  | ✅ RESOLVED — `BETTER_AUTH_URL.startsWith('https://')` 기반 |
| Q6  | `additionalFields.role` enum 동기화 방법 (수동 mirror + assert)     | W2-2 sub-step 1  | ✅ RESOLVED — Prisma SoT + boot-time `assertSameSet()` 가드 |
...
Plan 작성 후 Oracle 반복 라운드를 거치며 다음 부류의 issue들이 매번 새로 발견됨:
