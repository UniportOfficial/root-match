...
> Phase 3 (전자계약) vendor 결정 (UCanSign) 및 rootmatching 통합 design. 외부 vendor 공식 문서의 핵심 부분을 hoist하여 우리 시스템과의 매핑을 명시. 실제 구현은 W2-6 closure + Phase 2 (견적/매칭 persist) 후 별도 delegation으로 진행.
...
| 의존 컨텍스트    | PRD v0.4 §6.2 (전자계약) + §FR-5 / handoff §3.4 외부 dep #2 / backlog v0.7 §3.7                                                 |
...
1. **Vendor**: UCanSign 채택. 핵심 강점: webhook 4 events 명시 지원 + 가격 투명 (1건당 100원 + 신규 10건 무료) + embedding page (iframe 통합) 4 endpoints.
2. **API 적합성**: PRD FR-5 (계약) P0 cover + audit-trail PDF 지원. 단 한국 전자문서법 §4의2 감사추적인증서 명시는 audit-trail로 추정 (vendor 확인 needed). integrity_hash는 vendor 미명시 → rootmatching 자체 계산 패턴 (§4.4).
...
6. **Phase 5 알림 영향**: UCanSign 카카오 알림톡은 signingMethodType=kakao 선택 시 제한적 발송. 계약 완료 알림 + 비계약 거래 알림은 rootmatching 자체 통합 (Bizmsg/NHN; 외부 dep #6).
...
4 states. rootmatching `ContractStatus` Prisma enum:
...
→ webhook 받으면 즉시 매핑 가능 (별도 외부 lookup 불필요).
...
| FR-5.2 P0 | 전자서명 (vendor webhook)              | 4 webhook events 명시 (sign_creating / canceled / completed / completed_all) | ✅                                  |
...
| 부가      | integrity hash (SHA-256)               | 가이드 미명시                                                                | 🔴 vendor 미지원 → 자체 계산 (§4.4) |
| 부가      | embedding page (iframe 통합)           | 4 endpoints 명시 + redirectUrl + action 패턴                                 | ✅                                  |
...
UCanSign 카카오 알림톡은 `signingMethodType=kakao` 선택 시 vendor 발송 (제한적, 서명 요청 단계만).

**Phase 5 NotificationModule scope**:

- 계약 서명 요청 알림 = UCanSign `signingMethodType=kakao` 활용 (제한적 cover)
- 계약 완료 알림 + 비계약 거래 알림 (매칭 / 검수 / 결제 / 분쟁) = rootmatching 자체 통합 (Bizmsg/NHN)
- 외부 dep #6 (카카오 알림톡 비즈 계정) 그대로 필요
...
| 버전 | 날짜       | 변경                                                                                                                                                                                                                                                                                                                                                                |
| ---- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v0.1 | 2026-06-04 | 초기 작성. Phase 3 전자계약 vendor UCanSign 채택. 8 endpoint group (회원/문서/폴더/결제/포인트/탬플릿/임베딩페이지/웹훅) 매트릭스 + 통합 architecture (Prisma Contract + UCanSignClient + UCanSignAuthService + webhook handler) + Open questions 8건 vendor 문의 needed. 본 spec은 documentation only — 실제 구현은 Phase 3 진입 시 plan §A.7 delegation으로 진행. |
