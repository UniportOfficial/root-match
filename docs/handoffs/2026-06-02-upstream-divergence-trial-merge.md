...
## 9. 통합 완료 — Chunks 2~4 (v1.2, 2026-06-02)

`dev-monorepo`가 모든 upstream UI 자산을 React/Next.js로 흡수했습니다. `trial/merge-main-into-monorepo` 브랜치는 dev-monorepo가 통합본을 흡수했으므로 삭제됨.
...
- 검증 명령 모두 EXIT 0:
  - `pnpm install` (839 packages)
  - `pnpm --filter @rootmatching/shared run build`
  - `pnpm -r run typecheck`
  - `pnpm lint`
  - `pnpm -r run build`

### Chunk 2 (commits 61e6e8a + 73cb50e)

**Foundation (`61e6e8a`)**:

- Tailwind 3.4 + PostCSS + autoprefixer (Tailwind 4.x 미사용, 토큰 호환성)
- lucide-react, react-hook-form, @hookform/resolvers, zod, clsx, tailwind-merge
- `tailwind.config.ts` with design tokens from `docs/design-system.md`
  - brand DEFAULT=#2563eb, brand.hover, brand.light
  - ink.{950,700,400}, surface.{DEFAULT,muted}, border.{DEFAULT,subtle}
  - 의미 색상 success/warning/danger
- `globals.css` (@tailwind + Noto Sans KR), `layout.tsx`, `page.tsx` (랜딩)
- `lib/cn.ts` (clsx + tailwind-merge), `components/ui/AppBadge.tsx`
- `packages/shared/src/types/requests.ts` (ReceivedQuoteRequest, Attachment)
- `eslint.config.mjs`: `*.cjs` override (postcss.config.cjs용)

**Ports (`73cb50e`)**:

- `(client)/quotes/page.tsx` (218줄, 202줄 Vue에서 포팅) — 공개 견적 모집 게시판
- `(client)/request/page.tsx` (799줄, 811줄 Vue에서 포팅) — 새 견적 요청 폼
  - react-hook-form + zod
  - 인라인 커스텀 캘린더 위젯
  - 드래그/클릭 파일 업로드 (HTML5)
  - 포팅된 헬퍼: `buildEstimatedQuantity`, `buildBudgetRange`
  - 제출 시 POST `NEXT_PUBLIC_API_URL/matching/recommend`
- `(client)/matching/page.tsx` (555줄, 505줄 Vue에서 포팅) — AI 매칭 결과
  - sessionStorage hydration (1h staleness)
  - 4종 필터 + 선택 상태
- `apps/api/src/matching/matching.controller.ts` — POST /matching/recommend
- `apps/api/main.ts` — CORS + port 3001

### Chunk 3 (commit 90121b5)

**Types (`packages/shared/src/types/transactions.ts`)**:

- TransactionStatus, TransactionRole, TransactionFile, TransactionUpdate, TransactionCase

**Data (`apps/web/src/data/transactionData.ts`)**:

- transactionStatusStyles + transactionSteps + 5 mock TransactionCase 항목

**Ports (Sisyphus-Junior visual-engineering 위임)**:

- `(common)/transactions/page.tsx` (195줄) — 필터링 가능한 거래 목록
- `(common)/transactions/[id]/page.tsx` (423줄) — 진행 스텝퍼 + 납품/검수 파일 + 업데이트 타임라인
- `(client)/requests/page.tsx` (157줄) — 내 견적 요청 목록
- `(client)/requests/[id]/page.tsx` (320줄) — 견적 요청 상세

### Chunk 4 (commit 90121b5)

**Types (`packages/shared/src/types/disputes.ts`)**:

- DisputeStatus, DisputeStep, DisputeTimelineItem, DisputeCase

**Data (`apps/web/src/data/disputeData.ts`)**:

- disputeStatusStyles + 4 mock DisputeCase 항목

**Ports (Sisyphus-Junior visual-engineering 위임)**:

- `(common)/disputes/page.tsx` (179줄) — 상태 필터가 있는 분쟁 목록
- `(common)/disputes/[id]/page.tsx` (203줄) — 분쟁 상세 (스텝, 타임라인, 증빙 파일)
- `(common)/mypage/page.tsx` (539줄) — react-hook-form 기반 프로필 편집기, 다중 섹션, 컬러 피커, 강점/포트폴리오

### Final Build Output (12 routes)

```
/              3.42 kB         119 kB
/_not-found    0 B             115 kB
/disputes      8.78 kB         131 kB
/disputes/[id] 8.34 kB         131 kB
/matching      8.67 kB         131 kB
/mypage        4.49 kB         137 kB
/quotes        7.67 kB         130 kB
/request       22.6 kB         155 kB
/requests      7.15 kB         130 kB
/requests/[id] 8.6 kB          131 kB
/transactions  8.22 kB         131 kB
/transactions/[id] 10.3 kB     133 kB
```

총 9 static + 3 dynamic ([id]) 페이지. apps/web first-load ≤ 155 kB.

### 포팅 통계

| 메트릭                    | 값                                                 |
| ------------------------- | -------------------------------------------------- |
| Vue 원본 줄수 (포팅 대상) | 약 4,036 줄                                        |
| React 결과 줄수           | 약 4,005 줄 (page.tsx 합산)                        |
| 신규 shared 타입          | 14개 인터페이스 + 6개 union                        |
| 신규 mock data            | 4개 파일 (factory, request, transaction, dispute)  |
| 신규 NestJS API           | 1개 endpoint (/matching/recommend)                 |
| 처리 시간                 | 4개 visual-engineering 서브에이전트 약 20분 (병렬) |

### 남은 Phase 2 작업

| 항목                                         | 우선순위       | 메모                                                                                                    |
| -------------------------------------------- | -------------- | ------------------------------------------------------------------------------------------------------- |
| `/factory/requests/[id]` (공장 측 견적 상세) | Phase 2        | `apps/web/src/app/(client)/quotes/page.tsx` 내 마지막 1건 인터널 링크가 존재. 라우트 생성 시 자동 연결. |
| Prisma 6 + Neon 연결 + seed 마이그레이션     | Phase 1.Week 2 | apps/web/src/data/\* → apps/api/prisma/seed.ts                                                          |
| Better Auth 통합                             | Phase 1.Week 2 | MatchingGuard, 사용자 컨텍스트                                                                          |
| `/contract` 라우트 (matching → contract)     | Phase 2        | matching 페이지의 CTA가 가리킴                                                                          |
| sessionStorage → 영구 저장소                 | Phase 2        | matching/selected factory persistence를 DB로 이동                                                       |
| Factory-side 라우트 (`/factory/*`)           | Phase 2        | quote board의 상세, 견적 제출 등                                                                        |
...
| v1.2 | 2026-06-02 | Chunks 2~4 완료 — Tailwind foundation + 11 페이지 React 포팅 + AI API endpoint + 모든 verification 통과 + trial 브랜치 삭제 |
