...
export default function FactoryReceivedRequestsPage() {
      ...
      <div className="mx-auto max-w-7xl">
        <header className="mb-8 rounded-2xl border border-border bg-card p-6 shadow-ct-soft sm:p-8">
          ...
          <p className="text-kr-pretty mt-3 max-w-3xl text-base leading-8 text-muted-foreground sm:text-lg">
            발주처가 등록한 견적 요청을 검토하고 견적을 제출하세요.
          </p>
        </header>
          ...
          <CardContent className="p-4">
            <div className="grid grid-cols-1 gap-3 lg:grid-cols-[1fr_240px_180px]">
              <label className="relative block">
                <span className="sr-only">받은 견적 요청 검색</span>
        ...
        <main className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3">
          {filteredRequests.map((request) => (
            <Link key={request.id} href={`/factory/requests/${request.id}`} className="block">
              <Card className="h-full border-border bg-card shadow-ct-soft transition hover:border-brand-light hover:shadow-ct-card">
                <CardHeader className="p-4 pb-0 sm:p-5 sm:pb-0">
                  <div className="flex flex-col gap-4">
                    ...
                    <span className="text-kr-keep inline-flex items-center justify-center gap-2 rounded-lg border border-line-strong bg-surface px-3 py-2 text-[15px] font-bold text-foreground transition hover:border-brand-subtle hover:bg-brand-light/40 hover:text-brand">
                      상세 보기 및 견적 제출
