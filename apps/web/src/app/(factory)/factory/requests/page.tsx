        ...
        <main className="grid grid-cols-1 gap-5 xl:grid-cols-2">
          {filteredRequests.map((request) => (
            <Link key={request.id} href={`/factory/requests/${request.id}`} className="block">
              <article className="h-full rounded-2xl border border-border bg-white p-5 shadow-sm transition hover:border-brand-light hover:shadow-md sm:p-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <span className="inline-flex items-center justify-center gap-2 rounded-xl border border-border px-4 py-2.5 text-sm font-bold text-ink-700 transition hover:border-brand-light hover:bg-brand-light/40 hover:text-brand">
                    상세 보기 및 견적 제출
